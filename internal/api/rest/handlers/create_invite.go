...
func (a Adapter) CreateInvite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		a.log.WithError(err).Errorf("invalid user ID: %s", req.Data.Attributes.UserId)
	...
	invite, err := a.app.CreateInvite(r.Context(), app.CreateInviteParams{
	...
	if err != nil {
		...
		case errors.Is(err, errx.ErrorInitiatorIsNotThisDistributorEmployee):
			ape.RenderErr(w, problems.PreconditionFailed("initiator is not this distributor employee"))
	...
	a.log.Infof("invite sent successfully for user ID: %s in distributor ID: %s", userID, distributorID)
